...
import ChartRenderer from './chartRenderer.js'; // Corrected path assumption if needed, relative to chartController.js
...
export default class ChartController {
     ...
     * rendering, and live updates. It orchestrates interactions between the DataManager,
    ...
    constructor(container, announceEl, params, { logFn = console.log } = {}) {
        ...
        this.wsService = null; // WebSocketService instance for live data updates
        this.pollerId = null; // Interval ID for HTTP polling fallback (if WS fails)
        this.isLoadingHist = false; // Flag to prevent concurrent historical data loads via panning
        this.isLiveView = true; // Flag indicating if the chart view is currently at the latest data
        this.minPointsForZoom = 10; // Minimum number of data points required for initial zoom to be applied
        this.initialDataRendered = false; // Flag set after the very first data batch is successfully rendered
    ...
    _announce(msg, isError = false) {
        ...
        if (el) {
            el.textContent = ''; // Clear existing message immediately for responsiveness
            setTimeout(() => { if (el) el.textContent = msg; }, 50); // Set new message after a brief delay
        }
    }
    ...
    /**
     * Resolves or rejects the current initialization promise. This method is called internally
    ...
    _settleInitPromise(isSuccess, valueOrError) {
        this._clearActiveInitTimeout(); // Clear any pending timeout
        if (isSuccess) {
    ...
    /**
     * Initializes the chart controller for a new chart display. This method is the main entry point
     * for loading any chart configuration. It handles cleaning up previous chart state,
    ...
    async init() {
        ...
        this.wsService?.stop(); // Gracefully stop WebSocket connection
        clearInterval(this.pollerId); // Clear any active HTTP polling interval
        this.pollerId = null;
        this.wsService = null; // Clear reference to old WS service instance

        // The `uiBindings.js` (caller) is expected to have destroyed the *previous* ChartController instance,
        // which would have destroyed its `this.chart`. So, `this.chart` on *this new instance* should be null.
        ...
        this.initPromise = new Promise((resolve, reject) => {
            ...
            // Set a timeout for initial data to arrive and render.
            ...
            this.activeInitTimeoutId = setTimeout(() => {
                if (this.rejectCurrentInitPromise && !this.initialDataRendered) {
                    ...
                    this._error(errMsg);
                    this._settleInitPromise(false, new Error(errMsg)); // Reject the promise if timeout
                } else if (this.initialDataRendered && this.resolveCurrentInitPromise) {
                    // This scenario means data rendered just before timeout, but promise wasn't settled. Force resolve.
        ...
        try {
            ...
            this.initPromise = null; // Clear the promise reference regardless of outcome
        }
    }
        ...
        if (total >= this.minPointsForZoom) {
            ...
            if (typeof minTs === 'number' && typeof maxTs === 'number' && minTs <= maxTs) {
                if (minTs === maxTs && total > 1) {
                    ...
                    const windowMin = pointTime - (barDuration * 5); // 5 bars before
                    const windowMax = pointTime + (barDuration * 5); // 5 bars after
                    this.chart.xAxis[0].setExtremes(windowMin, windowMax, true, false); // Set extremes without immediate redraw
                    this._announce(`Showing single data point.`);
                } else {
                    // Normal zoom: set extremes for the calculated range
                    this.chart.xAxis[0].setExtremes(minTs, maxTs, true, false); // Set extremes without immediate redraw
    ...
    /**
     * Handles incoming WebSocket messages from the server. This method is the central dispatcher
            ...
            if (payload?.initial_batch) {
                ...
                if (ohlcData.length > 0) {
                    ohlcData.forEach((barArray, i) => {
                        ...
                        try {
                            this.dataManager.addBar(fullBarData, false); // Add without allowing updates for initial batch
                        } catch (e) {
                ...
                this.chart = this.renderer.render({ // ChartRenderer.render handles creation OR update
                    ...
                    title: chartTitle
                }, this.initialDataRendered); // Pass initialDataRendered: if true, attempt update


                if (this.chart) {
                    setChart(this.chart); // Store global reference to the chart instance for external tools
                    this.initialDataRendered = true; // Mark this controller's chart as initially rendered
                    this._applyInitialZoom(); // Apply initial zoom after data is in
                    renderSuccess = true;
                } else {
                ...
                if (newBarsAddedToDataManager > 0) {
                    ...
                    if (this.chart && this.initialDataRendered) {
                        this.log(`catch_up_batch: Refreshing chart data after ${newBarsAddedToDataManager} bars added/updated.`);
                        ...
                        this.chart.get('vol')?.setData(this.dataManager.fullVol, this.isLiveView); // Redraw based on live view state
                    }
    ...
    handleLiveBar(barData) {
        ...
        try {
            ...
            // Add/update bar in DataManager. `true` allows updating the last bar if timestamps match.
            ...
            if (updateType === 'updated') {
                ...
                if (ohlcSeries?.data.length > 0) {
                    ...
                    else ohlcSeries.addPoint(ohlcPoint, false, false, false); // Add if not last, but also not a truly new bar
                }
                ...
                const shift = (ohlcSeries?.data?.length || 0) > 2000; // Shift oldest point off if series gets too long (e.g., > 2000 points)
            ...
            this._saveLatestTsToLocalStorage(); // Save latest timestamp after a successful live update
        } catch (err) {
    ...
    startLive(clientSince = 0) {
        ...
        clearInterval(this.pollerId); // Clear any existing HTTP poller interval
        this.pollerId = null;
     ...
     * when the user scrolls towards the left (older data). This enables infinite scrolling for history.
    ...
    handlePan(event) {
        ...
        const axis = event.target; // The xAxis that triggered the event
        const extremes = axis.getExtremes(); // Current visible extremes (min, max, dataMin, dataMax)
            ...
            this.dataManager.loadMoreHistory()
                .then(historicalData => {
                    if (historicalData.ohlc.length > 0) {
                        ...
                        const preservedMin = Math.max(extremes.min, newChartExtremes.dataMin); // Ensure view doesn't go beyond new dataMin
                        const preservedMax = preservedMin + (extremes.max - extremes.min); // Maintain the visible range
                        axis.setExtremes(preservedMin, preservedMax, true, false); // Set new extremes without immediate redraw
        ...
        const dataMax = this.dataManager.latestTs ?? extremes.dataMax ?? extremes.max; // Get the most recent known data point's timestamp
        this.isLiveView = extremes.max >= dataMax - (this.dataManager.msPerBar || 60000) * 0.5; // Within half a bar of the latest data
    ...
    startPolling(pollSince = 0) {
        ...
        const intervalMs = Math.max(5000, Math.min(30000, (this.dataManager.msPerBar || 60000) / 2)); // Poll faster for smaller timeframes
        ...
        pollFn(); // Run immediately on start
        this.pollerId = setInterval(pollFn, intervalMs); // Schedule recurring polls
    }
    ...
    destroy() {
        ...
        this.wsService?.stop(); // Stop WebSocket service
        clearInterval(this.pollerId); // Clear HTTP poller
        this.pollerId = null;
        this.wsService = null; // Clear WS service reference
        ...
        if (this.chart) {
            ...
            this.chart = null; // Clear chart reference
            setChart(null); // Clear global chart reference managed by chartStore
        }
    ...
    _zoom(dir) {
        ...
        const newRangeFactor = dir === 'in' ? 0.7 : 1.3; // Zoom in by 30%, zoom out by 30%
        ...
        newRange = Math.max(newRange, minSensibleRange); // Ensure new range is at least minSensibleRange
        const maxDataRange = extremes.dataMax - extremes.dataMin; // Total span of available data

        if (dir === 'out' && newRange >= maxDataRange && range >= maxDataRange ) {
            if (extremes.min !== extremes.dataMin || extremes.max !== extremes.dataMax) {
                axis.setExtremes(null, null, true, true); // Full zoom out if not already at full extent
        ...
        newRange = Math.min(newRange, maxDataRange); // Ensure new range doesn't exceed total data span

        const center = extremes.min + range / 2; // Center of current view
        ...
        axis.setExtremes(newMin, newMax, true, true); // Set new extremes and trigger redraw
    ...
    _pan(dir) {
        ...
        const axis = this.chart.xAxis[0];
        const extremes = axis.getExtremes();
        const currentSpan = extremes.max - extremes.min; // Current visible time range
        ...
        const moveAmount = currentSpan * 0.25; // Pan by 25% of current visible range
        ...
        if (dir === 'left') {
            ...
            newMin = Math.max(extremes.dataMin, extremes.min - moveAmount); // Pan left, clamped by dataMin
            newMax = newMin + currentSpan; // Maintain current visible span
            newMax = Math.min(newMax, extremes.dataMax); // Ensure newMax doesn't go beyond dataMax
            ...
            const latestDataPoint = this.dataManager.latestTs ?? extremes.dataMax; // Use latest live TS or dataMax
            if (extremes.max >= latestDataPoint) { // Already at the rightmost data
                this._announce("Already at the newest data.");
                return;
            }
            newMax = Math.min(latestDataPoint, extremes.max + moveAmount); // Pan right, clamped by latestDataPoint
            newMin = newMax - currentSpan; // Maintain current visible span
            newMin = Math.max(newMin, extremes.dataMin); // Ensure newMin doesn't go below dataMin
            ...
            newMax = latestDataPoint; // Set max to latest live data point
            newMin = Math.max(extremes.dataMin, latestDataPoint - currentSpan); // Pan to show latest data, maintain span
        ...
        axis.setExtremes(newMin, newMax, true, true); // Apply new extremes and trigger redraw
