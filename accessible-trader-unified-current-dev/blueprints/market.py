...
import logging
from quart import Blueprint, request, current_app,g
...
from plugins.base import InstrumentTradingDetails, PluginError, MarketPlugin, PluginFeatureNotSupportedError
...
@market_blueprint.route("/<market>/<provider>/<path:symbol>/trading-details", methods=["GET"])
@jwt_required # <--- ADD @jwt_required DECORATOR
async def get_instrument_trading_details_endpoint(market: str, provider: str, symbol: str) -> tuple[Dict[str, Any], int]:
    """
    Fetches detailed trading rules and parameters for a specific instrument 
    for the authenticated user.
    Path parameter `symbol` can contain slashes.
    Query parameter `market_type` (e.g., 'spot', 'margin', 'future') can be used.
    """
    market_type: str = request.args.get("market_type", "spot").lower() # Default to spot
    
    # Get user_id from g, which is populated by @jwt_required
    user_id = g.user.get("id")
    if not user_id: # Should ideally be caught by @jwt_required if it returns an error response
        logger.error(f"/trading-details: User ID not found in g.user after @jwt_required for {market}/{provider}/{symbol}.")
        return make_error_response("Authentication context error. User ID missing.", code=401)

    log_prefix = f"/trading-details (User:{user_id}, MarketCat:{market}, Prov:{provider}, Sym:{symbol}, Type:{market_type}):"
    logger.info(f"{log_prefix} Request received.")

    if not all([market, provider, symbol]):
        return make_error_response("Missing one of required path parameters: market, provider, or symbol", code=400)

    try:
        svc: Optional[MarketService] = getattr(current_app, 'market_service', None)
        if not svc or not isinstance(svc, MarketService):
            logger.error(f"{log_prefix} MarketService not found.")
            return make_error_response("Internal server configuration error", code=500)

        plugin_instance = await svc.get_plugin_instance(market, provider, user_id=user_id) # Pass user_id
        
        supported_features = await plugin_instance.get_supported_features()
        if not supported_features.get("fetch_instrument_trading_details"):
            logger.warning(f"{log_prefix} Plugin does not support 'fetch_instrument_trading_details'.")
            return make_error_response(f"Fetching trading details is not supported by '{provider}'.", code=501)

        details: InstrumentTradingDetails = await plugin_instance.get_instrument_trading_details(
            symbol=symbol,
            market_type=market_type
        )
        
        if not details or not details.get("symbol"):
             logger.warning(f"{log_prefix} Plugin returned no/incomplete details for '{symbol}'.")
             return make_error_response(f"Could not retrieve trading details for '{symbol}'.", code=404)

        logger.info(f"{log_prefix} Successfully fetched details for '{symbol}'.")
        return make_success_response(data=details)

    # ... (keep your existing detailed exception handling) ...
    except ValueError as e_val: 
        logger.warning(f"{log_prefix} ValueError: {e_val}", exc_info=False)
        return make_error_response(f"Request error: {str(e_val)}", code=400)
    except PluginFeatureNotSupportedError as e_pfnse:
        logger.warning(f"{log_prefix} Feature not supported: {e_pfnse}")
        return make_error_response(str(e_pfnse), code=501)
    except AuthenticationPluginError as e_auth: # If plugin needs auth for this details call
        logger.warning(f"{log_prefix} Plugin Authentication Error: {e_auth}", exc_info=True)
        return make_error_response(f"Plugin authentication error: {str(e_auth)}", code=401)
    except PluginError as e_plugin:
        logger.error(f"{log_prefix} PluginError: {e_plugin}", exc_info=True)
        # Determine status code based on plugin error type if possible
        status_code = 502 
        if "Network" in e_plugin.__class__.__name__: status_code = 504
        return make_error_response(f"Provider error: {str(e_plugin)}", code=status_code)
    except Exception as e_critical:
        logger.exception(f"{log_prefix} Unexpected critical error: {e_critical}")
