...
class MarketService:
    ...
    async def _get_user_api_credentials(self, user_id: str | int | None, plugin_target_provider: str) -> Dict[str, Any | None]:
        """
        Fetches and decrypts user-specific API credentials from the user_configs_db asynchronously.

        Args:
            user_id (str | int | None): The ID of the user. If None, implies public access.
            plugin_target_provider (str): The specific provider (service_name) the credentials are for.

        Returns:
            Dict[str, Any | None]: A dict with 'api_key', 'api_secret', 'api_passphrase',
                                     and 'aux_data' (which is the parsed JSON from encrypted_aux_data).
                                     Values are None if not found or decryption fails.
        """
        if user_id is None:
            logger.debug(f"MarketService_GetUserCreds: No user_id provided for provider '{plugin_target_provider}'. Returning no keys.")
            return {'api_key': None, 'api_secret': None, 'api_passphrase': None, 'aux_data': None}

        log_prefix = f"MarketService_GetUserCreds (User: {user_id}, Provider: {plugin_target_provider}):"
        logger.debug(f"{log_prefix} Attempting to fetch API credentials asynchronously.")
        
        decrypted_creds = {'api_key': None, 'api_secret': None, 'api_passphrase': None, 'aux_data': None}
        
        try:
            query_user_id = int(user_id) # Ensure user_id is int for query

            async with user_configs_db_session_scope() as session: # Use async session scope
                stmt = (
                    select(UserApiCredential)
                    .where(UserApiCredential.user_id == query_user_id)
                    .where(UserApiCredential.service_name == plugin_target_provider.lower())
                    # Consider adding .where(UserApiCredential.is_active == True) if relevant
                )
                result = await session.execute(stmt)
                credential_record = result.scalars().first()

            # Decryption happens outside the DB session scope
            if not credential_record:
                logger.info(f"{log_prefix} No API credentials found for this user/provider combination.")
                return decrypted_creds

            # Decrypt core credentials. These are CPU-bound.
            # If performance becomes an issue with many decryptions, they could be run in threads:
            # e.g., decrypted_creds['api_key'] = await asyncio.to_thread(decrypt_data, credential_record.encrypted_api_key)
            
            decrypted_creds['api_key'] = decrypt_data(credential_record.encrypted_api_key)
            if not decrypted_creds['api_key']:
                logger.error(f"{log_prefix} Failed to decrypt API key for credential ID {credential_record.credential_id}. Aborting.")
                return {'api_key': None, 'api_secret': None, 'api_passphrase': None, 'aux_data': None}

            if credential_record.encrypted_api_secret:
                decrypted_creds['api_secret'] = decrypt_data(credential_record.encrypted_api_secret)
                if not decrypted_creds['api_secret']:
                    logger.warning(f"{log_prefix} Failed to decrypt API secret for credential ID {credential_record.credential_id}.")
            
            if credential_record.encrypted_aux_data:
                aux_data_plain = decrypt_data(credential_record.encrypted_aux_data)
                if aux_data_plain:
                    try:
                        parsed_aux_data = json.loads(aux_data_plain)
                        if isinstance(parsed_aux_data, dict):
                            decrypted_creds['aux_data'] = parsed_aux_data
                            decrypted_creds['api_passphrase'] = parsed_aux_data.get('api_passphrase')
                            logger.debug(f"{log_prefix} Decrypted and parsed aux_data. Passphrase found: {bool(decrypted_creds['api_passphrase'])}.")
                        else:
                            decrypted_creds['api_passphrase'] = aux_data_plain
                            decrypted_creds['aux_data'] = {'raw': aux_data_plain}
                            logger.debug(f"{log_prefix} Decrypted aux_data not dict, stored as direct api_passphrase/raw.")
                    except json.JSONDecodeError:
                        decrypted_creds['api_passphrase'] = aux_data_plain
                        decrypted_creds['aux_data'] = {'raw': aux_data_plain} # Store raw if not valid JSON string
                        logger.debug(f"{log_prefix} Decrypted aux_data not JSON, using as direct api_passphrase/raw.")
                else:
                    logger.warning(f"{log_prefix} Failed to decrypt auxiliary data for credential ID {credential_record.credential_id}.")

            logger.info(f"{log_prefix} API credentials processed (Key: {bool(decrypted_creds['api_key'])}, Secret: {bool(decrypted_creds['api_secret'])}, Passphrase: {bool(decrypted_creds['api_passphrase'])}).")
            return decrypted_creds

        except ValueError as ve: # Catching potential int(user_id) error
            logger.error(f"{log_prefix} Invalid user_id format: {user_id}. Error: {ve}", exc_info=True)
            return {'api_key': None, 'api_secret': None, 'api_passphrase': None, 'aux_data': None}
        except Exception as e:
            logger.error(f"{log_prefix} Error fetching/decrypting API credentials: {e}", exc_info=True)
            return {'api_key': None, 'api_secret': None, 'api_passphrase': None, 'aux_data': None} # Fail safe
